...
TAGS_TO_REMOVE = ["timestamp", "post_id", "id"]

TAGS_TO_RENAME = {"STEEM_": "HIVE_", "SBD_": "HBD_"}
...
class SimpleJsonTest:
    ...
    def compare_results(
        ...
        test_node_is_hive=TEST_NODE_IS_HIVE,
        reference_node_is_hive=REFERENCE_NODE_IS_HIVE,
    ):
        try:
            from requests import post

            print(f"Sending query:\n{self.json_pretty_string(args)}")
            ...
            if resp1.status_code != 200 or resp2.status_code != 200:
                print(
                    f"One of the nodes returned non 200 return codes. Test node: {resp1.status_code}, reference node: {resp2.status_code}"
                )
            ...
            if print_response:
                print(f"Response from test node at {self._test_node}:\n{self.json_pretty_string(json1)}")
                print(f"Response from ref node at {self._ref_node}:\n{self.json_pretty_string(json2)}")

            json1_error = json1.get("error", None)
            json2_error = json2.get("error", None)

            if json1_error is not None:
                print(f"Test node {self._test_node} returned error code")
                self.write_to_file(filename1, json1, self._test_node)

            if json2_error is not None:
                print(f"Reference node {self._ref_node} returned error code")
            ...
            if resp1.status_code != resp2.status_code or json1 != json2:
                ...
                if json_diff:
                    print(f"Differences detected in jsons: {self.json_pretty_string(json_diff)}")
            ...
            print(f"Exception during sending request: {ex}")
            return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test-node", dest="test_node", type=str, default="https://api.steem.house", help="IP address of test node"
    )
    parser.add_argument(
        "--test-node-is-hive", dest="test_node_is_hive", type=bool, default=True, help="Test node is hive node?"
    )
    parser.add_argument(
        "--ref-node", dest="ref_node", type=str, default="https://api.steemit.com", help="IP address of reference node"
    )
    parser.add_argument(
        "--ref-node-is-hive", dest="ref_node_is_hive", type=bool, default=False, help="Reference node is hive?"
    )
    parser.add_argument("--work-dir", dest="work_dir", type=str, default="./", help="Work dir")
    parser.add_argument(
        "--print-response", dest="print_response", type=bool, default=False, help="Print response from nodes"
    )

    args = parser.parse_args()
    tester = SimpleJsonTest(args.test_node, args.ref_node, args.work_dir)

    test_args = {"jsonrpc": "2.0", "id": 1, "method": args.api, "params": json.loads(args.params)}
