...
def map_votes(votes: list):
    ...
    for vote in votes:
        key = vote['voter']
        assert key not in mapped, "Duplicated vote"
        mapped[key] = vote
    return mapped
...
def map_posts(posts: list):
    ...
    for post in posts:
        ...
        assert key not in mapped, "Duplicated post"
        mapped[key] = post
    return mapped
    ...
    if 'active_votes' in post:
        ...
        for vote in post['active_votes']:
            ...
            if key in vote_map:
                ...
                print(f"No matching vote found for @{key}")
    ...
    if isinstance(orig, list):
        ...
        for post in orig:
            ...
            if key in post_map:
                ...
                print(f"No matching post found for @{key}")

    elif isinstance(orig, dict):
        assert isinstance(out, dict), "Incompatible .orig/.out files"
        pat = {}
        raise AssertionError('Processing of single post patterns not implemented yet')
    else:
        raise AssertionError('Unrecognized post format')
