...
def compare_response_with_pattern(
    response,
    method=None,
    directory=None,
    ignore_tags=None,
    ...
    if isinstance(response_json, dict) and all(
        key in response_json.keys() for key in ["code", "details", "hint", "message"]
    ):
        error = response_json
...
def compare_rest_response_with_pattern(
    response,
    method=None,
    directory=None,
