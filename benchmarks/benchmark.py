...
environment.add_argument(
    '-l', '--list', dest='list_csv', **BOOL_PARAM, help='if specified program will just list avaiable CSV files'
)
environment.add_argument(
    '-r',
    '--root-dir',
    ...
    help=f'path to root directory of tests_api project [default={DEFAULT_ROOT_DIR}]',
)
environment.add_argument(
    '-d',
    '--datadir',
    ...
    type=str,
    default='./wdir',
    help='defines path to workdir (path to this dir will alway be recreated) [default=./wdir]',
)
environment.add_argument(
    '-j',
    '--jmeter',
    ...
    help='path to jmeter executable [default=/usr/bin/jmeter]',
)
environment.add_argument(
    '-q',
    '--supr-errors',
    dest='supr_err',
    **BOOL_PARAM,
    help="if specified error messages of bad requests won't be printed",
)
environment.add_argument(
    '--skip-version-check',
    dest='skip_version',
    **BOOL_PARAM,
    help='if specified, `hive_api.get_version` call will not be performed',
)
...
benchmarking.add_argument(
    ...
    dest='api',
    type=str,
    default='account_history_api',
    help='specifies which API should be tested [default=account_history_api]',
)
benchmarking.add_argument(
    '-c',
    '--csv',
    dest='select_csv',
    type=str,
    default='perf_60M_heavy.csv',
    help='selected CSV FILENAME (use -l to list avaiable), [default=perf_60M_heavy.csv]',
)
benchmarking.add_argument(
    '-k',
    '--loops',
    ...
    help=f'amount of loops over CSV file per thread (if exceed, thread loop over the file again); for cl mode: pass -1 for infite loop [default={DEFAULT_LOOP_COUNT}]',
)
benchmarking.add_argument(
    '-t',
    '--threads',
    ...
    type=int,
    default=10,
    help='defines amount of threads to use during tests [default=10]',
)
benchmarking.add_argument(
    '--call-style',
    dest='call_style',
    type=str,
    default='old-style',
    help='defines calling style, performaed by jmeter [default=old-style]',
    choices=('old-style', 'new-style', 'postgres'),
)
...
connection.add_argument(
    ...
    type=int,
    default=8095,
    help='port to use during tests; set to 5432 for direct postgres query [default=8095]',
)
connection.add_argument(
    '-a',
    '--address',
    ...
    type=str,
    default='localhost',
    help='addres to connect during test [default=localhost]',
)
connection.add_argument(
    '--postgres',
    dest='postgres_url',
    ...
    help='if specified connection string, tests will be performed on postgres db [default=postgresql:///haf_block_log]',
)
connection.add_argument(
    '--postgres-schema',
    dest='schema',
    type=str,
    default='hive',
    help='specifies schema, where functions are placed [default=hive]',
)
...
analytics.add_argument(
    ...
    dest='ignore_br',
    **BOOL_PARAM,
    help='if specified script will ignore requests that failed, error message is still printed',
)
...
AVAILA_CSV = dict()
csv_regex = compile(r'(.*)_([0-9]+)M_(.*)\.csv')
for csv_file in CSV_DIR_PATH.glob('*.csv'):
    csv_filename = csv_file.name
    csv_filename_match = csv_regex.match(csv_filename)
    assert (
        csv_filename_match is not None
    ), f'invalid name of {csv_filename}, it should be <perf|cl>_<amount of blocks>M_<tag>.csv'
    csv_regex_group = csv_filename_match.group(1, 2, 3)
    AVAILA_CSV[csv_filename] = (
        csv_file,
        (CSV.MODE.PERF if csv_regex_group[0] == 'perf' else CSV.MODE.CL),
        *csv_regex_group[1:],
    )

# list avaiable CSV files and exit
if args.list_csv:
    res = "Found and avaiable csv's:" + '\n' * 2
...
if (
    match("postgresql://([a-zA-Z0-9_-]+)(:([a-zA-Z0-9_-]+))?@(.*)/(.*)", POSTGRES_URL) is not None
):  # Ex. postgresql://user:pass@127.0.0.1/my_database
    ...
    postgres_url_jmeter = (
        f'postgresql://{parsed_postgres_url.hostname}{parsed_postgres_url_port}{parsed_postgres_url.path}?{credentials}'
    )
elif match('postgresql:///(.*)', POSTGRES_URL):  # Ex. postgresql:///my_database
    postgres_url_jmeter = POSTGRES_URL.replace('/', '', -1)
else:
    raise AssertionError('cannot parse postgres url')
...
try:
    ...
    if not SKIP_VERSION:
        ...
        if 'result' not in version or 'commit' not in version['result']:
    ...
    JMETER = Popen(
        args=(
            ...
            '-n',
            '-t',
            OUT_JMX_PATH.as_posix(),
            '-l',
            JMETER_REPORT_OUT_FILE.as_posix(),
            '-p',
            PROPERTIES_FILES.as_posix(),
        ),
        env=jmeter_env,
    ...
    try:
        ...
        while JMETER.poll() is None and '... end of run' not in line:
            ...
            if CSV_MODE == CSV.MODE.CL and (jmeter_interrupt or LOOP_COUNT > 0):
                ...
                if not IGNORE_BAD_REQ:
                    raise AssertionError(
                        f'test failed, check logs in {DATADIR.as_posix()} for more informations. Fail detected on line: `{raw_line}`'
                    )

        label = line[label_idx]  # endpoint

        if (
            'pre_check' == line[label_idx] or 'SQL_validate' == line[label_idx]
        ):  # exclude pre_check from performance report
            continue
