...
def get_timestamp_path_and_time(cwd):
  start = time()
  rel_test_path = os.getenv("PYTEST_CURRENT_TEST").split("::")[0]
  rel_timestamp_path = "%s.timestamp.txt" %rel_test_path.split(".")[0]
  return os.path.join(cwd, rel_timestamp_path), start

def make_timestamp():
  with open(get_timestamp_path_and_time(os.getcwd())[0], "w") as f:
    f.write("%.20f" %(time()))
  return {"": ""}

def measure_request_execution_time(response):
  cwd = os.getcwd()
  timestamp_path, start = get_timestamp_path_and_time(cwd)
